  ...
  render() {
    ...
    const techs = this.projects[nr].techs.map(tech =>
      <div key={tech.name} className="tech">
        ...
        <div><FontAwesomeIcon icon={tech.img} size="2x" color={tech.color} /></div>
      </div >
    )
    ...
    return (
      <div className="containerProjekty">
        ...
        <div className="links">
          <a href={this.projects[nr].link} target="blank">Zobacz online</a>
          <a href={this.projects[nr].gitLink} target="blank">Kod na github</a>
        </div>
        <div className="nextProject">
          <button
            onClick={this.projectChange}>
            Nastepny projekt <FontAwesomeIcon icon={faForward} size="lg" />
          </button>
        </div>
      </div>
    );
